tmihoc | 2024-09-17 07:57:08 UTC | #1
...
In the charm SDK, in [Ops](/t/5527), Juju hooks are translated into Ops events = 'events', specifically, into classes that inherit from [`HookEvent`](https://ops.readthedocs.io/en/latest/index.html#ops.HookEvent).
...
In Juju, a **hook** is an executable file that a Juju unit agent runs to communicate with the [charm](/t/5457), in the follow sequence: 
...
- In the current, [Ops (`ops`)](/t/5527), framework, they are functions in a single Python file. 

In all implementations, h

Hooks form the basis of the [event](/t/6361)-handling system. 

In the current charm development framework, [Ops (`ops`)](/t/5527), events are code blocks, typically methods of a class that inherits from `CharmBase` and represents the charm object.
