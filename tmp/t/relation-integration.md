...
> See also: {ref}`How to manage relations <how-to-manage-relations>`

In Juju, a **relation** (**integration**) is a connection an {ref}`application <application>` supports by virtue of having a particular {ref}`endpoint <endpoint>`.

<!--
the connection of one {ref}`application <application>` to another (non-peer relation) or of one application to itself (peer relation) that is possible by virtue of the application's {ref}`endpoints <endpoint>`  and enables the applications to work together (non-peer relation) / persist data or scale (peer relations). 

As all operations in Juju, relations happen through the application {ref}`unit(s) <unit>`, with the {ref}`unit agent(s) <5464md>` polling the {ref}`controller agent <5464md>`. (In the case of relations, the controller agent acts as a message broker, within a virtual [star topology](https://en.wikipedia.org/wiki/Star_network).)
-->
<!--
a connection between two {ref}`applications <application>` with compatible {ref}`endpoints <endpoint>`  (i.e., the same endpoint interface and compatible endpoint roles -- `requires` to `provides`, `provides` to `requires`, or `peers` to `peers`) that enables them to notify each other of changes and to reconfigure their workloads to operate together, without manual intervention.
-->
...
- [Relation taxonomy](#heading--relation-taxonomy)
- [Relation identification](#heading--relation-identification)
- [Permissions around relation databags](#heading--permissions-around-relation-databags)
- [Relation lifecycle](#heading--relation-lifecycle)

<a href="#heading--relation-taxonomy"><h2 id="heading--relation-taxonomy">Relation taxonomy</h2></a>

- [Non-peer](#heading--non-peer)
    - [Non-subordinate](#heading--non-subordinate)	
         - [Same-model](#heading--same-model)
         - [Cross-model](#heading--cross-model)
     - [Subordinate](#heading--subordinate)
 - [Peer](#heading--peer)

<a href="#heading--non-peer"><h3 id="heading--non-peer">Non-peer</h3></a>
...
<a href="#heading--non-subordinate"><h4 id="heading--non-subordinate">Non-subordinate</h4></a>

A **non-subordinate** relation (aka 'regular') is a [non-peer](#heading--non-peer) relation where the applications are both principal. 

<a href="#heading--same-model"><h5 id="heading--same-model">Same-model</h5></a>

A **same-model** relation is a [non-subordinate](#heading--non-subordinate) relation where the applications are on  the same model.

<a href="#heading--cross-model"><h5 id="heading--cross-model">Cross-model</h5></a>
> See also: {ref}`How to manage relations > Add a cross-model relation <5464md>`


A **cross-model** relation (aka 'CMR') is a [non-subordinate](#heading--non-subordinate) relation where the applications are on different models (+/- different controllers, +/- different clouds).
...
<a href="#heading--subordinate"><h4 id="heading--subordinate">Subordinate</h4></a>

A **subordinate** relation is a [non-peer](#heading--non-peer) relation where one application is principal and the other subordinate.
...
<a href="#heading--peer"><h3 id="heading--peer">Peer</h3></a>
...
Because every relation results in the creation of unit and application databags in Juju's database, peer relations are sometimes used by charm authors as a way to persist charm data. When the application has multiple units, peer relations are also the mechanism behind {ref}`high availability <high-availability-ha>`.
...
<a href="#heading--relation-identification"><h2 id="heading--relation-identification">Relation identification</h2></a>
...
<a href="#heading--permissions-around-relation-databags"><h2 id="heading--permissions-around-relation-databags">Permissions around relation databags</h2></a>
...
<a href="#heading--relation-lifecycle"><h2 id="heading--relation-lifecycle">Relation lifecycle</h2></a>
...
<small> **Contributors:** @ppasotti, @serdarvural80, @tmihoc</small>