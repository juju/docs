...
If you want to control the amount of logging detail first, see {ref}`Manage the logging configuration <9151md>`.
...
> See also: {ref}`Kubernetes agent logs <9151md>`
...
----
```{dropdown} Expand to see examples
...
For complete syntax, see the {ref}`Command reference <9151md>` page.
...
----
```{dropdown} Example setting machine agent logs to `WARNING` and unit agent logs to `TRACE`
...
```
----
```{dropdown} Example setting unit agent logs for unit `0` of `mysql` to `DEBUG`
...
----
> See more: {ref}`How to configure a model <9151md>`, {ref}`List of model configuration keys > `logging-config` <9151md>`
...
> See more: {ref}`How to configure a controller <9151md>`
...
> See more: {ref}`How to configure a model <9151md>`
...
> See more: {ref}`Juju agent logs -- machines <9151md>`, {ref}`Juju agent logs -- Kubernetes <9151md>`
...
> See more: [How to access a machine via SSH <9151md>`
...
---
```{dropdown} Expand to see an example for a controller machine
...
> See also: {ref}`List of controller configuration keys <list-of-controller-configuration-keys>`
...
The full list of the controller settings that configure log file rotation is shown below. Normally these are set at bootstrap time with the `--config` option (see {ref}`How to configure a controller <9151md>`).
