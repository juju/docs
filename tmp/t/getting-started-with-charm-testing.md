(getting-started-with-charm-testing)=
# Getting started with charm testing
...
pengale | 2022-08-17 21:33:50 UTC | #3

Thank you for putting this together!

After an initial read through, I have the following comments:

1) The language is a little too informal. I think that we can tighten it up, while still keeping the tone approachable and friendly.

2) The intro to the unit testing section is overly long. I think that it could be rewritten to be much more concise. 

If I were to rewrite it, I would switch the focus from what the harness cannot do to what it enables. E.g., instead of talking about how some things must be mocked, talk about how the harness eliminates the need for mocking a lot of calls. That should both streamline the doc, and provide a more compelling case for writing tests with the harness. 

3) I would like to see examples of unit testing and integration testing throughout their respective sections. I think that it's a mistake to wait until the end of the unit testing section to show an example.

~ Penny

-------------------------

pedroleaoc | 2022-10-14 11:25:59 UTC | #4



-------------------------

pedroleaoc | 2022-10-14 11:30:16 UTC | #5



-------------------------

evschen | 2022-12-08 09:45:42 UTC | #6

I saw there it's said that 

[quote="ppasotti, post:1, topic:6894"]
the harness exposes a mock filesystem the charm will be able to interact with without knowing the difference
[/quote]

Could you provide some additional info for this? This would be helpful but I didn't find any relevant documentation on this mocking feature.

-------------------------

ppasotti | 2023-01-02 07:01:02 UTC | #7

Good starting point: [Container.push docs ](https://ops.readthedocs.io/en/latest/#ops.model.Container.push)

In a nutshell: 
- from test code, you can `harness.model.get_container('x').push(...)`, or pull, or list files, or anything you'd normally do with a container from charm code, but with god powers
- from charm code, you shouldn't have to change anything for things to 'work'. The charm can push, pull, etc... from all containers it has access to (after you've set them up with the harness).

For an example of testing code making use of containers: [grafana unittests](https://github.com/canonical/grafana-k8s-operator/blob/a6a5260ca2b9b3a31efc6ba6261dc9ba5bede36f/tests/unit/test_charm.py#L161)

-------------------------

erik-lonroth | 2023-06-25 10:59:54 UTC | #8

This is a fantastic write up. I've entered into the space a little and managed to create a *build->release->contious-integration* pipeline for the [Nextcloud charm](https://github.com/nextcloud-charmers/operator-nextcloud). It's still very basic (which I like).

 For reference, 

Here is the [Nextcloud release pipeline](https://github.com/nextcloud-charmers/operator-nextcloud/blob/72c9170cfa2f794785c9c236fe808be6804d5050/.github/workflows/release.yml ), using the [reusable workflows from the data-platform workflows](https://github.com/canonical/data-platform-workflows) at Canonical and @carlcsaposs . 

It will: 
1. Build the charm on a push to main branch.
2. Upload and create a new revision on the edge channel in charmhub.
3. Create a github release with the same revision number as the uploaded charm.

Here is the [Nextcloud Continous Integration (CI) pipeline](https://github.com/nextcloud-charmers/operator-nextcloud/blob/72c9170cfa2f794785c9c236fe808be6804d5050/.github/workflows/ci-nextcloud-postgresql.yaml) (manually triggered) using the "[actions-operator](https://github.com/charmed-kubernetes/actions-operator)"

It will:

1. Spawn a juju environment and a LXD controller (This is what the *actions-operator* do).
2. Deploy Nextcloud and Posgresql and relates them.
3. Wait for the Nextcloud to become "active" within a fixed timeframe.

Much help from @carlcsaposs (github actions) and @taurus (more github) and @tmihoc (docs) and the @ppasotti for this above. Also thanx for the hints from @barrettj12 about the various examples and workflows.