tmihoc | 2024-08-20 11:23:48 UTC | #1

<!-- NOTE: The spec information for validation should match what is in juju/charm/actions.go -->

> <small> [List of files in the charm project](/t/4454) > `actions.yaml`</small> 
...
[note type=caution]
:warning: Starting with Charmcraft 2.5, the `actions.yaml` file is created automatically from information you provide in the [`charmcraft.yaml`](/t/7132) file file. For backwards compatibility, Charmcraft will continue to allow the use of the `actions.yaml` file, but you may not duplicate keys across the two files. 
[/note]
...
---
[details=Expand to view the full spec at once]
```yaml
<action 1>:
  description: <string>
  parallel: <boolean>
...
[details=Expand to view a simple example]
...
[details=Expand to view a complex example]
...
---
[details=Expand to view an example featuring  `<action>.description` and `<action>.*`]
...
charlie4284 | 2024-08-01 07:52:04 UTC | #4

Hi! The actions link in 

> The `actions.yaml` file in a charm project is an optional file that may be used to define the [actions](https://discourse.charmhub.io/t/11149) supported by the charm.

seem to be broken, perhaps was it intended to be linked to https://juju.is/docs/juju/action? Not quite sure what the original post contained.

Thank you!

-------------------------

tmihoc | 2024-08-20 11:24:13 UTC | #5

Fixed, thanks! (And sorry it took so long.)
