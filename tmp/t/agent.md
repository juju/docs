...
In Juju, an **agent** is a {ref}``jujud` <binary-jujud>` / {ref}``containeragent` <binary-containeragent>` process that works to realise the state declared by a Juju end-user with a Juju client (e.g., {ref}`the Juju CLI <juju-juju-client>`) for a Juju entity (e.g., controller, model, machine, unit) via {ref}`workers <worker>`. 
...
<a href="#heading--controller-agent"><h2 id="heading--controller-agent">Controller agent</h2></a>
...
<a href="#heading--machine-agent"><h2 id="heading--machine-agent">Machine agent</h2></a>

On machine clouds, a `jujud` process running workers responsible for a {ref}`machine <machine>`.

<a href="#heading--model-agent"><h2 id="heading--model-agent">Model agent</h2></a>
...
<a href="#heading--unit-agent"><h2 id="heading--unit-agent">Unit agent</h2></a>
...
<!-- 
CONTENT THAT COULD BE MIGRATED TO A HOWTO PAGE
The agent's software version is generally consistent across a controller (and its models) and is thus determined at controller-creation time. By default the agent uses the same version as that of the local Juju client but this can be tweaked if desired. See {ref}`Agent versions and streams <5466md>` for how to do this.
