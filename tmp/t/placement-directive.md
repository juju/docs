...
In Juju, a **placement directive** is an option based on the `--to` flag that can be passed to certain commands to specify a deploy location, where the commands include {ref}``add-machine` <6187md>` ,  {ref}``add-unit` <6187md>` ,  {ref}``bootstrap` <6187md>` ,  {ref}``deploy` <6187md>` ,  {ref}``enable-ha` <6187md>` and the location is  (1) an existing or a new machine or (2) a key-value pair specifying a subnet, system ID, or an availability zone. 
...
> See more: {ref}`Machine designations <6187md>`
