...
## Usage
```juju wait-for {ref}`options] <command> ...```
...
## Subcommands
- [application <command-juju-wait-for-application>`
- {ref}`machine <command-juju-wait-for-machine>`
- {ref}`model <command-juju-wait-for-model>`
- {ref}`unit <command-juju-wait-for-unit>`