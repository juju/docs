...
> <small> {ref}`User <user>` > User access levels</small>
>
> See also: {ref}`How to manage a user's access level <how-to-manage-users>`
...
|---|---|---|
|<a href="#heading--controller-login"><h4 id="heading--controller-login">`login`</h4></a>|Via {ref}``juju register` <command-juju-register>`.|Log in to the controller.|
|<a href="#heading--controller-superuser"><h4 id="heading--controller-superuser">`superuser`</h4></a>|Automatically by bootstrapping a controller. <p> Automatically by having the username 'admin'. <p> Via {ref}``juju grant` <command-juju-grant>`.| You can do anything that it is possible to do at the level of a controller.|
...
|Access level|Granted|Abilities|
|---|---|---|
|<a href="#heading--cloud-add-model"><h4 id="heading--cloud-add-model">`add-model`</h4></a>|Via {ref}``juju grant-cloud` <command-juju-grant-cloud>`.|Add a model. <p> Grant another user model-level permissions.|
|<a href="#heading--cloud-admin"><h4 id="heading--cloud-admin">`admin`</h4></a>|Via {ref}``juju grant-cloud` <command-juju-grant-cloud>`.|You can do anything that it is possible to do at the level of a cloud.|
...
|Access level|Granted|Abilities|
|---|---|---|
|<a href="#heading--model-read"><h4 id="heading--model-read">`read`</h4></a>|Via {ref}``juju grant` <command-juju-grant>`.|View the content of a model without changing it. Can use any of the read commands.|
|<a href="#heading--model-write"><h4 id="heading--model-write">`write`</h4></a>|Via {ref}``juju grant` <command-juju-grant>`.|Deploy and manage applications on the model.|
|<a href="#heading--model-admin"><h4 id="heading--model-admin">`admin`</h4></a>|Via {ref}``juju grant` <command-juju-grant>`.|You can do anything that it is possible to do at the level of a model. |

<!--|<a href="#heading--model-consume"><h4 id="heading--model-consume">`consume`</h4></a>|Via {ref}``juju grant` <6864md>`.|Consume offers hosted by that model. No other access to the model.|-->
...
|Access level|Granted|Abilities|
|---|---|---|
| <a href="#heading--offer-read"><h4 id="heading--offer-read">`read`</h4></a>|Via {ref}``juju grant` <command-juju-grant>`.|View offers during a search with {ref}``juju find-offers` <command-juju-find-offers>`.|
| <a href="#heading--offer-consume"><h4 id="heading--offer-consume">`consume`</h4></a>|Via {ref}``juju grant` <command-juju-grant>`.|Relate an application to the offer.|
| <a href="#heading--offer-admin"><h4 id="heading--offer-admin">`admin`</h4></a>|Via {ref}``juju grant` <command-juju-grant>`.| You can do anything that it is possible to do at the level of an offer.|
...
See tutorial {ref}`Multi-user external setup <6864md>` for a quick walkthrough.
