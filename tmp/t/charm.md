...
A charm is software that responds to {ref}`hooks <5457md>` triggered by {ref}`agents <5457md>`.  Charms also define {ref}`actions <5457md>`, {ref}`relations <5457md>` and {ref}`metrics <5457md>`. 

There are two strategies for charm writing. The recommended approach is to use the reactive framework. An alternative strategy is to use individual scripts to respond to each {ref}`charm hook <5457md>`.  

<a id="charm-hook"></a><a id="charm-hooks"></a>
## Charm hook

Charm hooks are an important implementation detail of how {ref}`charms <5457md>` work. During the lifecycle of an application and its units, the controller execute files within a charm's `hooks` directory. 

Charm hooks have access to {ref}`hook tools <5457md>` for implementing their business logic.


<a id="hook-tools"></a>
## Hook tools

{ref}`Charm hooks <5457md>` interact with the {ref}`model <5457md>` via _hook tools_. They are utilities which  query and set  
