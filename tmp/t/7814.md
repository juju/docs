beliaev-maksim | 2024-08-30 06:23:34 UTC | #1

> <small> [From Zero to Hero: Write your first Kubernetes charm](/t/7113) > Observe your charm with COS Lite </small>
>
> **See previous: [Expose your charm's operational tasks via actions](/t/7592)** 
...
> Read more: [`provides`](/t/5213#heading--provides)
...
Since our application is configurable (see previous chapter [Make the charm configurable](/t/7401)) we need to define a dynamic port based on configuration setting, thus, you can see that target job is set to be consumed from config. Prometheus library automatically appends the URL path (which defaults to `/metrics`), so, we do not need to include it here. 
...
<!--
According to the documentation of the Loki charm, we require the [`loki_push_api`]( https://charmhub.io/loki-k8s/libraries/loki_push_api) library.
-->
...
> Read more: [`requires`](/t/5213#heading--requires)
...
> **See next: [Write units tests for your charm](/t/11961)**
...
beliaev-maksim | 2022-12-15 16:46:54 UTC | #2



-------------------------

beliaev-maksim | 2023-01-31 16:48:40 UTC | #4



-------------------------

gboutry | 2023-02-06 10:07:47 UTC | #5

In section about fetching the `juju_topology` library, the command has an additional `charmcraft fetch-lib`

Current command:
```
ubuntu@charm-dev:~/fastapi-demo$ charmcraft fetch-lib charmcraft fetch-lib charms.observability_libs.v0.juju_topology
```
...
Moreover, Grafana dashboard download link leads to a 404 error:
https://juju.is/uploads/short-url/6hSGcAA6n20qyStzLFeekgkzqCc.tmpl

-------------------------

beliaev-maksim | 2023-02-06 11:38:35 UTC | #6

fixed, thanks

-------------------------

bartz | 2023-07-18 10:57:33 UTC | #7

I think the Note

> Note: The `loki_push_api` library also depends on the [`juju_topology`](https://charmhub.io/observability-libs/libraries/juju_topology) library, but you have already fetched it above for Prometheus.

in [Fetch the Grafana Interface libraries](https://discourse.charmhub.io/t/observe-your-charm-with-cos-lite/7814#heading--fetch-the-grafana-interface-libraries)
should rather be:
...
beliaev-maksim | 2023-07-23 11:39:00 UTC | #8

@tmihoc can you please have a look

-------------------------

javierdelapuente | 2023-11-21 14:33:24 UTC | #9

The big image of the grafana dashboard makes the page https://juju.is/docs/sdk/observe-your-charm-with-cos-lite fail with 500.

Not sure if the best solution would be to resize the image (so there is not preview mode) or to update canonicalwebteam.discourse project, so it does not fail.

![image|690x525](upload://q79AKrTIeHPSjzykA3FcEIHjDKQ.png)

-------------------------

tmihoc | 2023-11-21 15:12:11 UTC | #10

Thanks for bringing this up. It definitely used to work. @mthaddon do you think it could be related to the recent Discourse redeployment?

-------------------------

javierdelapuente | 2023-11-21 15:14:54 UTC | #11

By the way, the same problem happens in more places, see https://juju.is/docs/sdk/talking-to-a-workload-control-flow-from-a-to-z

-------------------------

tmihoc | 2023-11-21 15:16:24 UTC | #12

Then it's definitely related to the recent Discourse redeployment. Because all of these docs used to work fine. Thanks again -- I'll look into this!

-------------------------

tmihoc | 2023-11-21 16:10:20 UTC | #13

Should be fixed soon: https://github.com/canonical/canonicalwebteam.discourse/pull/179

Thanks a lot, @Lukewh !

-------------------------

Lukewh | 2023-11-21 20:30:12 UTC | #14

Apologies for the delay we had some infrastructure woes, but this is now deployed and the issue should be resolved :)!

-------------------------

mylesjp | 2024-04-05 20:55:32 UTC | #15

There are a few old references to `metadata.yaml` which as of Charmcraft 2.5 should be `charmcraft.yaml`:

> In your `metadata.yaml` file, before the `peers` block, add a `provides` endpoint with relation name `metrics-endpoint` and interface name `prometheus_scrape`, as below. This declares that your charm can offer services to other charms over the `prometheus-scrape` interface. In short, that your charm is open to integrations with, for example, the official Prometheus charm. (Note: `metrics-endpoint` is the default relation name recommended by the `prometheus_scrape` interface library.)

> Now, in your charm’s `__init__` method, initialise the `MetricsEndpointProvider` instance with the desired scrape target, as below. Note that this uses the relation name that you specified earlier in the `metadata.yaml` file.

> In your `metadata.yaml` file, beneath your existing `requires` endpoint, add another `requires` endpoint with relation name `log-proxy` and interface name `loki_push_api`

> Then, in your charm’s `__init__` method, initialise the `LogProxyConsumer` instance with the defined log files, as shown below. The `log-proxy` relation name comes from the `metadata.yaml` file and the`demo_server.log` file is the file where the application dumps logs. Overall this code ensures that your application can push logs to Loki (or any other charms that implement the `loki_push_api`).

> In your `metadata.yaml` file, add another `provides` endpoint with relation name `grafana-dashboard` and interface name `grafana_dashboard`, as below.

> Now, in your charm’s `__init__` method, initialise the `GrafanaDashboardProvider` instance, as below. The `grafana-dashboard` is the relation name you defined earlier in your `metadata.yaml` file. Overall, this code states that your application supports the Grafana interface.
