...
The _model_ is the central abstraction provided by Juju. Models house {ref}`applications <5456md>`, manage their {ref}`relations <5456md>`. Models allow a set of applications to be treated as a single entity.
...
A model maintains exclusive access to the {ref}`machines <5456md>` that its applications' units use.   
...
The {ref}`model <5456md>` created as part of the {ref}`bootstrap <5456md>` process.
-->

> See also:
> - {ref}`How to manage models <how-to-manage-models>`
> - {ref}`About application modelling <about-application-modelling>`
...
A model is created by a {ref}`user <user>`, and owned in perpetuity by that user (or a new user with the same name), though it may also be used by any other user with a {ref}`model access level <5456md>`, within the limits of their level. 
...
> See more: {ref}`List of model configuration keys <list-of-model-configuration-keys>`,  {ref}`How to configure a model <5456md>`