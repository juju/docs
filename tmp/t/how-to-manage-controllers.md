...
> See also: {ref}`Controller <controller>`

```{important}

To be able to manage a controller, a user must have a {ref}`controller `superuser` <1111md>` access level.
...
> See also: {ref}`Bootstrapping <bootstrapping>`, {ref}`List of supported clouds > \<cloud name\> > CLOUD <list-of-supported-clouds>`
...
> See more: [List of controller configuration keys > `caas-image-repo` <1111md>` <br>:warning: While this key *can* technically be changed after bootstrap, that is only for a very specific use case (adjusting credentials used for a custom registry). For most cases it is safe to assume you can only set it during bootstrap.
...
> See more: {ref}``juju bootstrap` <command-juju-bootstrap>`
...
> See more: [How to manage the client <how-to-install-and-manage-the-client>`
...
> See more: [How to manage machine constraints for a controller <1111md>`, {ref}`How to make the controller highly available <1111md>`
...
> See more: {ref}`How to add a cloud <1111md>`
...
> See more: {ref}``juju bootstrap --keep-broken` <command-juju-bootstrap>`
...
> See more: {ref}`How to manage logs > View the log files <1111md>`, {ref}`How to troubleshoot your deployment <how-to-troubleshoot-your-deployment>`
...
> See more: [`juju controllers` <command-juju-controllers>`
...
> See more: [`juju show-controller` <command-juju-show-controller>`
...
> See also: [Configuration <configuration>`, {ref}`List of controller configuration keys <list-of-controller-configuration-keys>`
>
> See related: {ref}`How to configure a model <1111md>`
...
> See more: [`juju bootstrap --config` <command-juju-bootstrap>`, {ref}``juju controller-config` <command-juju-controller-config>`
...
> See more: {ref}``juju controller-config` <command-juju-controller-config>`
...
> See more:
> - [How to manage constraints for a model <1111md>`
> - {ref}`How to manage constraints for an application <1111md>`
...
> See more: [How to add a user <1111md>`
...
> See more: {ref}`How to register a private controller  <1111md>`
...
> See more: [`juju register` <command-juju-register>`, {ref}`How to add a user <1111md>`
...
> See more: {ref}``juju register` <command-juju-register>`
...
> See more: {ref}``juju unregister` <command-juju-unregister>`
...
> See also: [High availability <high-availability-ha>`
...
> See more: [`juju enable-ha` <command-juju-enable-ha>`
...
> - {ref}`How to switch to a different model <1111md>`
> - {ref}`How to add a cross-model relation <1111md>`
...
> See more: [`juju create-backup` <command-juju-create-backup>`
...
> See more: {ref}``juju download-backup` <command-juju-download-backup>`
...
* `--manual-agent-control`: (in the case of restoring backups to {ref}`high availability controllers <1111md>`) stop and restart `juju` agents and Mongo daemons on the secondary controller machines manually
...
See [How to upgrade your deployment <how-to-upgrade-your-deployment>` for a full demo!

```

> See more: 
> - {ref}`How to back up a controller > Create a backup, Restore from a backup <1111md>`
> - {ref}`How to migrate a workload model to another controller <1111md>`
> - {ref}`How to manage a user's login details <1111md>`
...
Additionally, if there is persistent {ref}`storage <1111md>` in any of the controller's models you will be prompted to either destroy or release the storage, using the `--destroy-storage` or `--release-storage` options respectively.
...
from being destroyed. A controller administrator is able to enable all the commands across all the models in a Juju controller so that the controller can be destroyed if desired. This can be done via the {ref}``enable-destroy-controller` <1111md>` command: `juju enable-destroy-controller`.

```

> See more: {ref}``juju destroy-controller` <command-juju-destroy-controller>`
...
> See more: {ref}``juju kill-controller` <command-juju-kill-controller>`
