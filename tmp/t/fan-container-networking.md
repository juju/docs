...
<!-- TOC -->
* {ref}`Fan overview <1065md>`
* {ref}`Juju model Fan configuration <1065md>`
* {ref}`Cloud provider requirements <1065md>`
* {ref}`Examples <1065md>`
    * {ref}`Rudimentary confirmation of the Fan using a GCE cloud <1065md>`
    * {ref}`Deploying applications with the Fan using an AWS cloud <1065md>`
...
See {ref}`Configuring models <1065md>` for more details on setting model options.
...
You may need to create a new VPC if you are using an old AWS account (the original VPC may be deficient). Some may simply prefer to have a Juju-dedicated VPC. See appendix [Creating an AWS VPC <1065md>` for instructions.

```

Whether you created a secondary VPC out of necessity or preference you will need to inform Juju about it. See {ref}`AWS specific features <1065md>` for how to do this.
