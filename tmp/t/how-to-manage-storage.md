...
The above only works if the volume is in the same availability zone as the instance. This is a requirement that comes from the `ebs` storage provider. See [Dynamic storage <dynamic-storage>`.

```

> See more: {ref}``juju add-storage` <command-juju-add-storage>`, {ref}`Storage constraint (directive) <storage-constraint-directive>`
...
> See more: {ref}``juju deploy --storage` <command-juju-deploy>`
...
> See more: [`juju storage` <command-juju-storage>`
...
> See more: [`juju show-storage` <command-juju-show-storage>`
...
> See more: [`juju detach-storage` <command-juju-detach-storage>`, {ref}`Dynamic storage <dynamic-storage>`
...
> See more: [`juju deploy --attach-storage` <command-juju-deploy>`
...
> See more: {ref}``juju add-unit --attach-storage` <command-juju-add-unit>`
...
> See more: {ref}``juju attach-storage` <command-juju-attach-storage>`
...
> See also: [`juju remove-storage` <5892md>`, {ref}`Removing things reference <removing-things>`
...
> See more: [`juju remove-storage` <command-juju-remove-storage>`
...
> See more: {ref}``juju destroy-model --destroy-storage` <command-juju-destroy-model>`
...
> See more: {ref}``juju destroy-controller --destroy-storage` <command-juju-destroy-controller>`
...
> See more: [`juju import-filesystem` <command-juju-import-filesystem>`, {ref}`Storage provider > `lxd` <5892md>`
