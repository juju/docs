...
><small> {ref}`Storage <storage>` > Storage provider </small> 
>
> See also: {ref}`How to manage storage <how-to-manage-storage>`
...
> See also: {ref}`Persistent storage and Kubernetes <7181md>`
...
The regular package archives for Ubuntu 14.04 LTS (Trusty) and Ubuntu 16.04 LTS (Xenial) do not include a version of LXD that has the 'lxd' storage provider feature. You will need at least version 2.16. See the {ref}`Using LXD with Juju <the-lxd-cloud-and-juju>` page for installation help.
...
See {ref}`Using LXD with Juju <the-lxd-cloud-and-juju>` for how to use LXD in conjunction with Juju, including the use of ZFS as an alternative filesystem.
