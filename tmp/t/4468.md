jnsgruk | 2024-04-25 13:55:05 UTC | #1
...
> See more: [File `charmcraft.yaml` > `resources`](/t/7132#heading--resources)
...
The [`fetch()`](https://ops.readthedocs.io/en/latest/#ops.Resources.fetch) method will raise a [`ModelError`](https://ops.readthedocs.io/en/latest/#ops.ModelError) if the resource does not exist, and returns a Python [`Path`](https://docs.python.org/3/library/pathlib.html#pathlib.Path) object to the resource if it does.
...
> See more: [`charmcraft upload-resource`](/t/6139)
...
> See more: [`charmcraft resources`](/t/6136)
...
> See more: [`charmcraft resource-revisions`](/t/6135)
...
> See more: [`charmcraft set-resource-architectures`](/t/13046)

-------------------------

aurelien-lourot | 2021-06-03 12:23:10 UTC | #2
...
jnsgruk | 2021-06-03 13:28:06 UTC | #3
...
axino | 2023-01-05 20:19:29 UTC | #5

This should explain how to use an OCI image without uploading a charm to the charmhub (when doing local tests for example). The only "documentation" I could find was in https://bugs.launchpad.net/juju/+bug/1958253/comments/1

-------------------------

ppasotti | 2023-02-08 13:03:58 UTC | #6

I think we should also have a section on how to attach an image from a local registry to a charm.
something like `juju deploy ./mycharm.charm --resource foo=local-registry:foo-image:42.2.0`
