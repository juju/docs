tmihoc | 2024-08-29 15:33:03 UTC | #1

> See also: [How to manage relations](/t/1073)

In Juju, a **relation** (**integration**) is a connection an [application](/t/5471) supports by virtue of having a particular [endpoint](/t/5462).

<!--
the connection of one [application](/t/5471) to another (non-peer relation) or of one application to itself (peer relation) that is possible by virtue of the application's [endpoints](/t/5462)  and enables the applications to work together (non-peer relation) / persist data or scale (peer relations). 

As all operations in Juju, relations happen through the application [unit(s)](/t/5460), with the [unit agent(s)](/t/5466#heading--unit-agent) polling the [controller agent](/t/5466#heading--controller-agent). (In the case of relations, the controller agent acts as a message broker, within a virtual [star topology](https://en.wikipedia.org/wiki/Star_network).)
-->
<!--
a connection between two [applications](/t/5471) with compatible [endpoints](/t/5462)  (i.e., the same endpoint interface and compatible endpoint roles -- `requires` to `provides`, `provides` to `requires`, or `peers` to `peers`) that enables them to notify each other of changes and to reconfigure their workloads to operate together, without manual intervention.
-->
...
> See also: [How to manage relations > Add a cross-model relation](/t/1073#heading--add-a-cross-model-relation)
...
Because every relation results in the creation of unit and application databags in Juju's database, peer relations are sometimes used by charm authors as a way to persist charm data. When the application has multiple units, peer relations are also the mechanism behind [high availability](/t/1066).
...
pmatulis | 2023-01-20 19:30:36 UTC | #4

[quote="tmihoc, post:1, topic:5464"]
an **integration** is a protocol
[/quote]

I'm pretty sure that this is being confused with an *interface*. An interface is a protocol.

-------------------------

mthaddon | 2024-03-26 17:13:44 UTC | #5

[quote="tmihoc, post:1, topic:5464"]
each one of its units gets a databag in the Juju database
[/quote]

Is there a limit to the amount of data that can be passed in a relation databag?

-------------------------

tmihoc | 2024-04-11 07:17:10 UTC | #6

@mthaddon sorry for the very late reply -- I missed this somehow. The limit (for all keys + values in a relation database) is apparently 16MB.

-------------------------

mthaddon | 2024-04-11 08:13:21 UTC | #7

Great, thanks. Could this be added to the documentation itself? Is this 16MB per databag (e.g. each unit databag could have 16MB and the application databag could have 16MB) or 16MB per relation?

-------------------------

tmihoc | 2024-04-11 08:53:15 UTC | #8

@mthaddon :

@jameinel was saying:

> The "document" size limit is the limit for all keys + values in a relation database (of 16MB). So I'm guessing it's per relation? I'll investigate further and then update the doc.

-------------------------

tmihoc | 2024-04-11 14:50:08 UTC | #9

@mthaddon : @jameinel says: 

> I would caution trying to eek out too much from the system, but I'm pretty sure the application record is separate from the per-unit ones.
>
> This will likely change a bit in 4.0, though we'll try to be roughly comparable.

So the limit is per databag?

-------------------------

javierdelapuente | 2024-04-21 09:01:35 UTC | #10

[quote="tmihoc, post:1, topic:5464"]
For example, if the `wordpress/0*` leader uses `relation-set` to write to its local *unit* databag, its peers and the `mysql/0*` leader will receive `http-relation-changed`, because they can access that data (by calling `relation-get`), but the other mysql units will not be notified.
[/quote]

Isn't it the other way around? That is, when `wordpress/0*` uses `relation-set` to its local *unit* databag, it is all the `mysql` units that are notified?

-------------------------

javierdelapuente | 2024-04-22 09:15:11 UTC | #11
...
Checking manually, I believe there is a missing point, `all units of an application can read all the local unit's databags`.

-------------------------

tmihoc | 2024-04-25 11:27:50 UTC | #12

While I think that's true, I am told that's true accidentally rather than by design. But I'll double-check. PS Also looking into the point you were making about events in your previous comment.
