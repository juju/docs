beliaev-maksim | 2024-09-10 04:47:24 UTC | #1

> <small> [From Zero to Hero: Write your first Kubernetes charm](/t/7113) > Preserve your charm's data </small>
>
> **See previous: [Integrate your charm with PostgreSQL](/t/7414)** 
...
> Read more: [StoredState](/t/4456#heading--stored-state), [StoredState: Uses, Limitations](/t/5740)
...
```
> Read more: [File ‘charmcraft.yaml’](/t/7132#heading--peers-provides-requires)
...
> **See next: [Expose your charm's operational tasks via actions](/t/7592)**
...
beliaev-maksim | 2022-11-16 10:30:57 UTC | #2



-------------------------

beliaev-maksim | 2023-01-31 16:48:50 UTC | #3



-------------------------

mylesjp | 2024-04-05 20:45:48 UTC | #4

There are a few references to `metadata.yaml` in this document that should be changed to `charmcraft.yaml` with the latest changes in Charmcraft 2.5.

> ## [Define a peer relation](https://juju.is/docs/sdk/preserve-your-charms-data#heading--define-a-peer-relation)
> 
> The first thing you need to do is define a peer relation. Update the `metadata.yaml` file to add a `peers` block before the `requires` block, as below (where `fastapi-peer` is a custom name for the peer relation and `fastapi_demo_peers` is a custom name for the peer relation interface):
> 
> ```
> peers:
>   fastapi-peer:
...
> ```
> 
> > Read more: [File ‘metadata.yaml’](https://juju.is/docs/sdk/metadata-yaml)

> This block uses the built-in `json` module of Python, so you need to import that as well. You also need to define a global variable called `PEER_NAME = "fastapi-peer"`, to match the name of the peer relation defined in `metadata.yaml` file. At the beginning of your file, right after the import section, add all of the following:
