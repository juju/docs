...
> See {ref}`Set up your development environment automatically <12435md>` for instructions on how to set up your development environment so that it's ready for you to test-deploy your charm. At the charm directory step, call it `microsample-vm`. At the cloud step, choose LXD. 
...
> See more: {ref}`How to set up a charm project <how-to-set-up-a-charm-project>`, {ref}`List of files in a charm project <list-of-files-in-a-charm-project>`
...
> See more: {ref}`File `charmcraft.yaml` <file-charmcraftyaml>`
...
> See more: {ref}`File `src/charm.py` <file-srccharmpy>`, {ref}`Ops <ops-ops>`, {ref}`Event `start` <event-start>`, {ref}`Event `install` <event-install>`
...
> See more: {ref}`How to pack a charm <how-to-pack-a-charm>`
...
> See more: {ref}`File `charmcraft.yaml` > Key `config` <12435md>`
...
```
> See more: [Event `config-changed` <event-config-changed>`
...
> See more: [File `requirements.txt` <file-requirementstxt>`, [PyPI > Library `requests`](https://pypi.org/project/requests/), [PyPI > Library `requests-unixsocket`](https://pypi.org/project/requests-unixsocket/)
...
> See {ref}`Set up your development environment automatically > Clean up <12435md>`
