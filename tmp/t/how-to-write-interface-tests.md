...
> - {ref}`How to test interface tests <12690md>`

Suppose you have an interface specification in {ref}``charm-relation-interfaces` <charm-relation-interfaces>`, or you are working on one, and you want to add {ref}`interface tests <12690md>`. These are the steps you need to take.

We will continue from the running example from the previous HowTo on {ref}`How to register an interface <how-to-register-an-interface>`.
