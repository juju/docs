...
In Juju, you can use hook tools for troubleshooting.
...
See [Network primitives <1163md>` for in-depth coverage.
...
The environment variable {ref}``JUJU_REMOTE_UNIT` <1163md>` stores the default remote unit.

You should never depend upon the presence of any given key in `relation-get` output. Processing that depends on specific values (other than `private-address`) should be restricted to {ref}`-relation-changed <1163md>` hooks for the relevant unit, and the absence of a remote unit's value should never be treated as an {ref}`error <1163md>` in the local unit.

In practice, it is common and encouraged for {ref}`-relation-changed <1163md>` hooks to exit early, without error, after inspecting `relation-get` output and determining the data is inadequate; and for {ref}`all other hooks <1163md>` to be resilient in the face of missing keys, such that -relation-changed hooks will be sufficient to complete all configuration that depends on remote unit settings.
...
### `unit-get`
> :warning: `unit-get` is deprecated in favour of `network-get` hook tool. See [Network primitives <1163md>` for details.
...
<small><br> **Contributors:** @achilleasa, @amandahla, @benhoyt, @charlie4284, @dmitrii, @jameinel, @pmatulis, @ppasotti, @timclicks, @tmihoc  </small>