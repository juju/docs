...
> See also: {ref}`How to manage secrets <how-to-manage-secrets>`
...
- (starting with Juju 3.3) a {ref}`user <user>` with {ref}`model `admin` access <7286md>` (in which case the secret is also known as a 'user secret'). <br>
...
<!--
A secret can be created by charm code running on a unit vs.  a {ref}`model `admin` user <7286md>` (what is known as a 'user secret') and has:
...
> Read more: {ref}``charmcraft.yaml` / config <7286md>`
