tmihoc | 2024-08-22 15:43:18 UTC | #1

<!--
> - [`juju bootstrap --bootstrap-constraints`](/t/1688)
> - [`juju bootstrap --constraints`](/t/1688)
> - [`juju deploy --constraints`](/t/1707)
> - [`juju add-machine --constraints`](/t/1672)
> - [`juju constraints`](/t/7271)
> - [`juju model-constraints`](/t/7306)
> - [`juju set-constraints`](/t/1807)
> - [`juju set-model-constraints`](/t/1813)
-->

<!--REMOVING BECAUSE IT'S DEPRECATED:
> - [How to manage constraints](/t/5312)
-->
...
For machine (non-Kubernetes) clouds, constraints can be set directly on individual [machines](/t/5459). However, more commonly they are set at the level of the [controller](/t/5455), [model](/t/5456), or [application](/t/5471). If you set constraints at multiple levels at once -- that is, with overlap -- the constraint applied at the more specific level takes precedence.

The rest of this document describes all the existing constraints.

[note type=caution]
Some of these keys -- their availability and their meaning -- vary from one cloud to another. Below this is indicated with a generic note. For specifics see [List of supported clouds > `<cloud name>`](/t/6665).
[/note]
...
[note]**Note:** `root-disk-source` is the juju storage pool for the root disk. By specifying a storage pool, the root disk can be configured to use encryption.[/note]
...
[note]**Note:** `root-disk-source` is the LXD storage pool for the root disk. The default LXD storage pool is used if root-disk-source is not specified.[/note]
...
[note]**Note:** `root-disk-source` is either "local" or "volume"[/note]
...
[note]**Note:** `root-disk-source` is the datastore for the root disk[/note]
...
-   [Controller machine](#setting-constraints-for-a-controller)
-   All models
-   [Single model](#setting-and-displaying-constraints-for-a-model)
-   [Single application](#setting-displaying-and-updating-constraints-for-an-application)
-   All units of an application
-   [Single machine](#setting-constraints-when-adding-a-machine)
...
freyes | 2022-07-27 13:45:07 UTC | #3

Hi, 
just a nitpick, the constraint "cpu-cores" is an alias of "cores", using "cpu-cores" works but gives this warning:
...
tmihoc | 2022-07-27 14:11:03 UTC | #4

Fixed it now, thanks!

-------------------------

ghibourg | 2022-09-17 01:29:00 UTC | #5

There is a typo in the constraint `allocate-public-id`, it should be `allocate-public-ip`.

-------------------------

tmihoc | 2022-09-19 13:41:56 UTC | #6

Fixed, thanks!

-------------------------

pedroleaoc | 2022-10-14 11:30:13 UTC | #7



-------------------------

gschiano | 2023-01-17 10:59:25 UTC | #8
...
```
Is misleading, it seems to say that only `requests` are set on the pod, which is not what the ticket says (it says that limit = request and we can't set different values)
I also validated that `limit` is also set when a memory constraint is set:
```
    Limits:
      memory:  4Gi
    Requests:
...
lathiat | 2023-04-19 08:27:06 UTC | #9
...
thorsten-merten | 2023-07-04 08:57:36 UTC | #10

Hi there. I noticed that for MAAS the `tag` constraint seems to be missing. See e.g. this discussion on how it can be used https://discourse.charmhub.io/t/how-to-use-maas-constraints/5253

-------------------------

charlie4284 | 2023-11-23 02:31:21 UTC | #11

Hello!
The `virt-type` options can be updated to "container" or "virtual-machine".
```
...
aieri | 2024-06-21 23:27:03 UTC | #12

Since in many cases a user will want to leverage multiple constraints, it would be good for the docs to clarify that the correct syntax is to separate key/value pairs with a space (and quote the whole string).
In other words:
* `--constraints="key1=value1,value2 key2=value3` ← correct
* `--constraints key1=value1 --constraints key2=value2` ← incorrect, only `key2=value2` will be considered
