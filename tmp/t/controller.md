...
> See also: {ref}`How to manage controllers <how-to-manage-controllers>`
...
- It is set up via the [bootstrap <bootstrapping>` process. <p>
- It refers to the initial controller {ref}`unit <unit>` as well as any units added later on (for machine clouds, for the purpose of {ref}`high-availability <high-availability-ha>`) -- each of which includes 
    - a {ref}`unit agent <5455md>`, 
    - [`juju-controller`](https://charmhub.io/juju-controller) charm code, 
    - a {ref}`controller agent <5455md>` (running, among other things, the Juju API server), and 
