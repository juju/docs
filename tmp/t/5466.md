tmihoc | 2024-09-03 10:56:30 UTC | #1

In Juju, an **agent** is a [`jujud`](/t/7319) / [`containeragent`](/t/11672) process that works to realise the state declared by a Juju end-user with a Juju client (e.g., [the Juju CLI](/t/5465)) for a Juju entity (e.g., controller, model, machine, unit) via [workers](/t/11298). 
...
On machine and Kubernetes clouds, a `jujud` process running workers responsible for a [controller](/t/5455). This includes, among others, the `apiserver` worker, which is responsible for running the Juju API server.


<a href="#heading--machine-agent"><h2 id="heading--machine-agent">Machine agent</h2></a>

On machine clouds, a `jujud` process running workers responsible for a [machine](/t/5459).

<a href="#heading--model-agent"><h2 id="heading--model-agent">Model agent</h2></a>

On machine and Kubernetes clouds, a `jujud` process running workers responsible for all the [models](/t/5456) associated with a given controller.


<a href="#heading--unit-agent"><h2 id="heading--unit-agent">Unit agent</h2></a>

On machine / Kubernetes clouds, a `jujud` / `containeragent` process responsible for a [unit](/t/5460).
...
Agents run trees of [workers](https://juju.is/docs/dev/worker). These may have different areas of responsibility, at the level of a [machine](/t/5459), a [unit](/t/5460), or a [controller](/t/5455). As such, agents are sometimes also classified in those terms, that is, into *machine* agents, *unit agents*, and *controller* agents.
