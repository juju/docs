...
export CHARM_LAYER_PATH=$JUJU_REPOSITORY/layers
export CHARM_INTERFACE_PATH=$JUJU_REPOSITORY/interfaces

mkdir -p $CHARM_LAYER_PATH $CHARM_INTERFACE_PATH
```
...
The export of `CHARM_INTERFACE_PATH` is an environment variable which tells the
...
```Bash
mkdir -p $CHARM_INTERFACE_PATH/http
```
