...
```bash
cd ~
tar -cpzf juju-client-$(date "+%Y-%b-%d_%H-%M-%S").tar.gz .juju 
```
...
node) in case of failure. Included commands allow you to create, restore and 
manage backup file containing the state server
